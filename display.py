...
def set_camera(dispW=1280 , dispH=720, flip=0, fps=30):
    camSet='nvarguscamerasrc !  video/x-raw(memory:NVMM), width='+str(dispW)+', height='+str(dispH)+', format=NV12, framerate='+str(fps)+' ! nvvidconv flip-method='+str(flip)+' ! video/x-raw, width='+str(dispW)+', height='+str(dispH)+', format=BGRx ! videoconvert ! video/x-raw, format=BGR ! appsink'
    return cv2.VideoCapture(camSet)

def display_cam(cam, window_name, img_function, wait=1, **kwargs):
    while True:
        ret, frame = cam.read()
        img = img_function(**kwargs)
